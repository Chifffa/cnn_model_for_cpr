...
from cpr_cnn_model import get_model
from config import WEIGHTS_PATH, INPUT_NAME, OUTPUT_NAME
...
def convert(weights: Optional[str]) -> None:
    ...
    frozen_model_path = freeze_keras_model(model, save_path, os.path.splitext(weights_name)[0], [OUTPUT_NAME])
    convert_frozen_graph_to_onnx(frozen_model_path, [INPUT_NAME], [OUTPUT_NAME], save_path, opset=None)
